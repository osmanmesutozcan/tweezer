...
window.VirtualScroll = (function (document) {
    ...
    var initListeners = function () {
        // (passiveSupported && tive) ? el.addEventListener(name, fn, {
        //     passive: false,
        //     capture: bubble
        // }) : el.addEventListener(name, fn, bubble || false);

        const options = {passive: false, capture: true}
        if (hasWheelEvent) document.addEventListener("wheel", onWheel, options);
        if (hasMouseWheelEvent) document.addEventListener("mousewheel", onMouseWheel, options);
        ...
        if (hasPointer && hasTouchWin) {
            ...
            document.addEventListener("MSPointerDown", onTouchStart, options);
            document.addEventListener("MSPointerMove", onTouchMove, options);
        }

        if (hasKeyDown) document.addEventListener("keydown", onKeyDown, options);
